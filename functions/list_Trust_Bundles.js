...
if(type === "bundles"){
pagination_trust_bundles(page_url, subaccountSID);
}

if(type === "assignments"){
pagination_trust_bundle_assignments(page_url, subaccountSID,bundle)
}


async function pagination_trust_bundles(response, subaccountSID) {
 ...
 const token =  await clienttemp.api.v2010.accounts(subaccountSID)
  ...
  if (response === "0") {
    url = "https://trusthub.twilio.com/v1/TrustProducts?PageSize=20&Page=0";
  }
  else{
    url = response
  }
...
async function pagination_trust_bundle_assignments(response, subaccountSID,bundle) {
  const accountSid = process.env.ACCOUNT_SID;
  const authToken = process.env.AUTH_TOKEN;
 ...
 const token =  await clienttemp.api.v2010.accounts(subaccountSID)
  ...
  if (response === "0") {
    url =  `https://trusthub.twilio.com/v1/TrustProducts/${bundle}/ChannelEndpointAssignments?PageSize=20&Page=0`;
  }
  else{
    url = response
  }
